...
export function DepositScreenView({ open, onClose, initialCurrency = 'USDT', onOpenDepositHistory, onBackToHome }: DepositScreenViewProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {!onOpenDepositHistory && !onBackToHome && <span style={{ width: 40 }} />}
          </div>
        </div>
      ...
      {/* Mobile: floating Submit Deposit button pops up when proof is uploaded */}
      {isApiConfigured() && proofFile && (
        <div className="deposit-mobile-submit-popup">
          <button
            type="button"
            onClick={canSubmit ? handleSubmit : undefined}
            disabled={!canSubmit || submitting}
            style={{
              ...
              borderRadius: 12,
              border: 'none',
              background: canSubmit ? 'var(--accent)' : 'rgba(255,255,255,0.2)',
              color: canSubmit ? '#fff' : 'var(--text-muted)',
              fontSize: 15,
              fontWeight: 700,
              letterSpacing: '0.04em',
              cursor: canSubmit && !submitting ? 'pointer' : 'not-allowed',
              boxShadow: canSubmit ? '0 4px 20px rgba(34,197,94,0.4)' : 'none',
            }}
          >
